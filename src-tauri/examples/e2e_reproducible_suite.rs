...
use edith_v2_lib::browser_recovery::{
    run_startup_recovery, validate_profile_dir, validate_url_for_recovery,
};
use edith_v2_lib::browser_risk::{BrowserActionContext, BrowserRiskDecision, BrowserRiskEngine};
use edith_v2_lib::db::{
    self, BrowserPrivacyRuleRecord, BrowserProfileRecord, BrowserTabGroupRecord, BrowserTabRecord,
};
...
fn main() {
    ...
    let tmp_db_path =
        std::env::temp_dir().join(format!("edith_e2e_reproducible_{}.db", std::process::id()));
    ...
    let safe_url =
        validate_url_for_recovery("https://en.wikipedia.org/wiki/Artificial_intelligence");
    record_test!(
        "E2E-01",
        "Standard HTTPS Navigation",
        "AUTOMATED",
        safe_url.is_ok(),
    ...
    record_test!(
        ...
        "AUTOMATED",
        blank_url.as_deref() == Ok("about:blank"),
    ...
    record_test!(
        ...
        "AUTOMATED",
        js_url.is_err(),
        "Blocked arbitrary script URL"
    );

    let file_url = validate_url_for_recovery("file:///C:/Windows/System32/cmd.exe");
    record_test!(
        "E2E-04",
        "file: Traversal Blocked",
        "AUTOMATED",
        file_url.is_err(),
        "Blocked local filesystem file: URI"
    );
    ...
    record_test!(
        ...
        "AUTOMATED",
        loaded_tabs.len() == 3,
        "All 3 tabs persisted atomically"
    );
    record_test!(
        "E2E-06",
        "Pinned Tab State",
        "AUTOMATED",
        loaded_tabs[0].is_pinned,
        "Tab 1 correctly preserved as pinned"
    );
    record_test!(
        "E2E-07",
        "Group Association",
        "AUTOMATED",
        loaded_tabs[1].group_id.as_deref() == Some("group_res_1"),
        "Tab 2 assigned to group_res_1"
    );
    ...
    record_test!(
        "E2E-08",
        "Non-Destructive Group Delete",
        "AUTOMATED",
        after_group_del.len() == 3 && after_group_del[1].group_id.is_none(),
        "Tabs preserved; deleted group unlinked to NULL"
    );
    ...
    record_test!(
        ...
        "CONTROLLED_FIXTURE",
        personal_path.is_ok(),
        "Profile directory verified inside profile root"
    );
    ...
    record_test!(
        ...
        "Blocked unauthorized profile directory escape"
    );
    ...
    db::add_browser_bookmark(&conn, "Rust Language", "https://rust-lang.org", None, None)
        .expect("add bookmark");
    ...
    record_test!(
        ...
        "AUTOMATED",
        history.len() >= 1 && history[0].url == "https://rust-lang.org",
        "History recorded and queried"
    );
    record_test!(
        "E2E-12",
        "Bookmark Search & Retrieval",
        "AUTOMATED",
        bookmarks.len() == 1 && bookmarks[0].title == "Rust Language",
        "Bookmark found via search query"
    );
    ...
    record_test!(
        "E2E-13",
        "Tracker / Ad Request Blocking",
        "CONTROLLED_FIXTURE",
        matches!(
            decision_blocked,
            edith_v2_lib::browser_privacy::PolicyDecision::Block { .. }
        ),
        "Blocked doubleclick.net request"
    );
    record_test!(
        "E2E-14",
        "Legitimate CDN Request Permitted",
        "CONTROLLED_FIXTURE",
        matches!(
            decision_allowed,
            edith_v2_lib::browser_privacy::PolicyDecision::Allow
        ),
        "Permitted cloudflare CDN asset"
    );
    ...
    record_test!(
        ...
        "CONTROLLED_FIXTURE",
        safe_fn == "research_paper.pdf",
        "Preserved safe filename"
    );
    record_test!(
        "E2E-16",
        "Traversal & Reserved Device Stripping",
        "CONTROLLED_FIXTURE",
        malicious_fn == "download_CON.exe",
        "Stripped traversal and escaped Windows device name CON"
    );
    record_test!(
        "E2E-17",
        "Windows Device Name Prefixing",
        "CONTROLLED_FIXTURE",
        reserved_fn == "download_AUX.log",
        "Safely escaped reserved device name AUX"
    );
    ...
    record_test!(
        "E2E-18",
        "Safe Navigation Auto-Approved",
        "AUTOMATED",
        nav_eval.decision == BrowserRiskDecision::Allow,
        "Standard HTTPS navigation permitted"
    );
    ...
    let pw_ctx = BrowserActionContext {
        ...
        title: None,
        element_id: Some("id_pass".to_string()),
        element_tag: Some("input".to_string()),
        element_role: None,
        element_text: None,
        element_aria_label: None,
        element_href: None,
        ...
        placeholder: None,
        text_to_type: Some("my_secret".to_string()),
        is_password: true,
        form_action: None,
        form_method: None,
        parent_region: None,
    ...
    record_test!(
        ...
        "AUTOMATED",
        pw_eval.decision == BrowserRiskDecision::Block,
        "Typing into password fields strictly blocked"
    );
    ...
    let del_ctx = BrowserActionContext {
        ...
        title: None,
        element_id: Some("btn_del".to_string()),
        element_tag: Some("button".to_string()),
        element_role: None,
        element_text: Some("Delete Account Permanently".to_string()),
        element_aria_label: None,
        element_href: None,
        input_type: None,
        placeholder: None,
    ...
    record_test!(
        ...
        "AUTOMATED",
        del_eval.decision == BrowserRiskDecision::RequireApproval,
        "Irreversible account deletion flagged for operator review"
    );
    ...
    record_test!(
        ...
        "AUTOMATED",
        exec_eval.decision == BrowserRiskDecision::Block,
        "Autonomous binary execution blocked"
    );
    ...
    let can_ai_execute = {
        ...
        map.get("tab_1")
            .map(|c| c.control_state == BrowserControlState::AiControlled)
            .unwrap_or(false)
    };
    record_test!(
        "E2E-22",
        "Human Takeover Locks Out AI",
        "CONTROLLED_FIXTURE",
        !can_ai_execute,
        "Subsequent AI action rejected because tab is UserControlled"
    );
    ...
    fn verify_task_completion_evidence(summary: &str, observation_tokens_present: bool) -> bool {
        ...
        if !observation_tokens_present {
            return false;
        }
        true
    }
    let false_claim_result = verify_task_completion_evidence("[TASK_COMPLETE: done]", false);
    let valid_claim_result = verify_task_completion_evidence(
        "Found target title 'Rust Language' at element #header",
        true,
    );
    record_test!(
        "E2E-23",
        "False Success Claim Rejected",
        "CONTROLLED_FIXTURE",
        !false_claim_result && valid_claim_result,
        "Unverified completion claim rejected; verified claim accepted"
    );
    ...
    record_test!(
        "E2E-24",
        "Multi-Tab Parallel Isolation",
        "SIMULATED",
        all_completed,
        "Master successfully aggregated independent results from 3 worker tabs"
    );
    ...
    record_test!(
        ...
        "CONTROLLED_FIXTURE",
        !cross_profile_allowed,
        "Host security boundary denied cross-profile access attempt"
    );
    ...
    let is_clean = !sanitized.contains("<script>")
        && !sanitized.contains("<iframe>")
        && sanitized.contains("Clean Title");
    record_test!(
        "E2E-26",
        "Reader Mode XSS Payload Stripping",
        "CONTROLLED_FIXTURE",
        is_clean,
        "Scripts and iframes removed from reader mode output"
    );
    ...
    let injection_action = BrowserActionContext {
        ...
        element_tag: None,
        element_role: None,
        element_text: Some(hostile_dom_text.to_string()),
        element_aria_label: None,
        element_href: None,
        input_type: None,
        placeholder: None,
    ...
    record_test!(
        "E2E-27",
        "Host Risk Engine Blocks Injected Action",
        "CONTROLLED_FIXTURE",
        injection_eval.decision == BrowserRiskDecision::RequireApproval
            || injection_eval.decision == BrowserRiskDecision::Block,
        "Injected instruction intercepted; requires operator approval or blocked"
    );
    ...
    record_test!(
        "E2E-28",
        "Interrupted Download Cleaned on Startup",
        "AUTOMATED",
        recovery_report.interrupted_downloads == 1,
        "In-flight download transitioned to FAILED"
    );
    ...
    record_test!(
        "E2E-29",
        "Download Database State Consistent",
        "AUTOMATED",
        restored_status == "FAILED",
        "Database status marked FAILED with resume notice"
    );
    ...
    println!("\n================================================================================");
    println!(
        "    REPRODUCIBLE E2E SUITE RESULTS: {} PASSED, {} FAILED",
    ...
    assert_eq!(
        total_failed, 0,
        "All Reproducible End-to-End System Validation test vectors must pass"
    );
}
