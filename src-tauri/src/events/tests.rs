...
fn test_event_id_uniqueness() {
    ...
    for _ in 0..10_000 {
        ...
        assert!(
            set.insert(id.0),
            "Collision detected in EventId generation!"
        );
    }
...
fn test_stream_lifecycle_success() {
    ...
    // 1. Started
    emitter
        .emit_stream_started(&correlation, "llama-3.3-70b-versatile")
        .unwrap();

    // 2. Chunks (sequence 1, 2, 3)
    emitter
        .emit_stream_chunk(&correlation, "The", 1, false)
        .unwrap();
    emitter
        .emit_stream_chunk(&correlation, " quick", 2, false)
        .unwrap();
    emitter
        .emit_stream_chunk(&correlation, " brown", 3, true)
        .unwrap();

    // 3. Finished
    emitter
        .emit_stream_finished(&correlation, Some(3), Some("stop".to_string()))
        .unwrap();
...
fn test_stream_lifecycle_failed() {
    ...
    emitter
        .emit_stream_started(&correlation, "gemini-2.5-flash")
        .unwrap();
    emitter
        .emit_stream_chunk(&correlation, "Incomplete...", 1, false)
        .unwrap();
    emitter
        .emit_stream_failed(
            &correlation,
...
fn test_concurrent_stream_isolation() {
    ...
    let handle_a = std::thread::spawn(move || {
        ...
        emitter_a
            .emit_stream_finished(&corr_a, Some(50), None)
            .unwrap();
    });
    ...
    let handle_b = std::thread::spawn(move || {
        ...
        emitter_b
            .emit_stream_finished(&corr_b, Some(50), None)
            .unwrap();
    });
    ...
    for env in &stream_a_events[1..51] {
        match &env.payload {
            EdithPayload::Stream(StreamPayload::Chunk {
                ...
                assert_eq!(
                    env.correlation.conversation_id.as_deref(),
                    Some("session-1")
                );
...
fn test_cancellation_isolation() {
    ...
    // Both start
    emitter
        .emit_stream_started(&corr_active, "model-1")
        .unwrap();
    emitter
        .emit_stream_started(&corr_cancelled, "model-2")
        .unwrap();

    // Stream 1 emits chunk
    emitter
        .emit_stream_chunk(&corr_active, "Token 1", 1, false)
        .unwrap();

    // Stream 2 is cancelled
    emitter
        .emit_stream_cancelled(&corr_cancelled, Some("User cancelled".to_string()))
        .unwrap();

    // Stream 1 continues unaffected
    emitter
        .emit_stream_chunk(&corr_active, "Token 2", 2, true)
        .unwrap();
    emitter
        .emit_stream_finished(&corr_active, Some(2), Some("stop".to_string()))
        .unwrap();
