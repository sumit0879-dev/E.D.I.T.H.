...
impl CommandPolicy {
    ...
    pub fn parse_and_validate(raw_cmd: &str) -> Result<(String, Vec<String>), String> {
        ...
        {
            return Err(
                "Security Policy Violation: Direct invocation of shell interpreters is prohibited."
                    .to_string(),
            );
        }
        ...
        match program {
            ...
            "whoami" => {
                ...
                if args.is_empty() || (args.len() == 1 && allowed.contains(&args[0].as_str())) {
                    ...
                    Err(
                        "Invalid arguments for whoami. Permitted: [] or [/user, /groups, /priv]."
                            .to_string(),
                    )
                }
            }
            ...
            "dir" => Ok(("low", false)),
            _ => Err(format!(
                "Program '{}' is not in the approved execution policy catalog.",
                program
            )),
        }
    }
    ...
    pub fn execute(
        ...
        let output = cmd
            .output()
            .map_err(|e| format!("Process execution failed for '{}': {}", program, e))?;
        ...
        if stderr.len() > 1024 * 1024 {
            stderr =
                stderr.chars().take(1024 * 1024).collect::<String>() + "\n[Error Truncated at 1MB]";
        }
...
impl ProposalEngine {
    ...
    pub fn resolve_proposal(
        ...
        let proposal = store
            .get_mut(proposal_id)
            .ok_or_else(|| format!("Proposal '{}' not found or already purged.", proposal_id))?;
        ...
        if trimmed_session.is_empty() {
            return Err(
                "Security Authorization Error: Session ID is required and cannot be empty."
                    .to_string(),
            );
        }
        if proposal.session_id != trimmed_session {
            return Err(
                "Security Authorization Error: Session mismatch for command proposal.".to_string(),
            );
        }
        ...
        if now > proposal.expires_at {
            ...
            return Err(
                "Security Authorization Error: Command proposal has expired (TTL 300s)."
                    .to_string(),
            );
        }
        ...
        let current_hash =
            Self::compute_hash(&proposal.program, &proposal.args, &proposal.working_dir);
        ...
        if !action.eq_ignore_ascii_case("approve") {
            return Err(format!(
                "Invalid proposal action '{}'. Expected 'Approve' or 'Reject'.",
                action
            ));
        }
        ...
        if let Some(c) = conn {
            if let Ok(custom_apps) = crate::db::get_custom_apps(c) {
                for app in custom_apps {
                    if app.name.eq_ignore_ascii_case(trimmed)
                        || app.path.eq_ignore_ascii_case(trimmed)
                    {
                        return match open::that(&app.path) {
                ...
                if canonical.starts_with(&canonical_root) {
                    ...
                    if let Ok(meta) = std::fs::metadata(&canonical) {
                        if meta.len() > 5 * 1024 * 1024 {
                            return Err("File is too large for real-time preview (exceeds 5MB)."
                                .to_string());
                        }
                    }
                    return Ok(canonical);
...
impl CredentialVault {
    ...
    pub fn protect(_plaintext: &str) -> Result<String, String> {
        Err(
            "Security Error: CredentialVault DPAPI is supported on Windows desktop targets only."
                .to_string(),
        )
    }
    ...
    pub fn unprotect(ciphertext: &str) -> Result<String, String> {
        ...
        unsafe {
            ...
            if res != 0 && !out_blob.pbData.is_null() {
                let slice = std::slice::from_raw_parts(out_blob.pbData, out_blob.cbData as usize);
                let text =
                    String::from_utf8(slice.to_vec()).map_err(|e| format!("UTF8 error: {}", e))?;
                LocalFree(out_blob.pbData as *mut std::ffi::c_void);
                Ok(text)
            } else {
...
impl LanceDbSanitizer {
    pub fn sanitize_source_predicate(source: &str) -> Result<String, String> {
        ...
        if source.len() > 256 {
            return Err(
                "Source metadata string exceeds maximum allowed length of 256 characters."
                    .to_string(),
            );
        }
...
mod tests {
    ...
    fn test_sec02_proposal_lifecycle() {
        ...
        let proposal = ProposalEngine::create_proposal("session_test", "whoami", &[], &dir, "low");
        assert_eq!(proposal.status, ProposalStatus::Pending);

        // Approve proposal with correct session
        let res =
            ProposalEngine::resolve_proposal(&proposal.proposal_id, "session_test", "Approve");
        assert!(res.is_ok());
    ...
    fn test_sec02_empty_or_wrong_session_rejected() {
        let dir = std::env::current_dir().unwrap();
        let proposal = ProposalEngine::create_proposal(
            "session_valid",
            "echo",
            &["test".to_string()],
            &dir,
        ...
        assert!(ProposalEngine::resolve_proposal(
    ...
    fn test_sec03_path_sandbox_containment() {
        ...
        if cargo_toml.exists() {
            let res =
                PathSandbox::verify_containment(&cargo_toml.to_string_lossy(), &allowed_roots);
            assert!(res.is_ok());
        }
