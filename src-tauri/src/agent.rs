...
You can only use one tool at a time. Do not write anything after the tool block. Wait for the user or system to provide the result.");
    ...
    for _ in 0..max_iterations {
        ...
        let mut loop_correlation =
            crate::events::EventCorrelation::for_task("dev_agent", session_id.clone());
        ...
        let ai_reply = if let Some(streamer) = adapter.as_streaming_text() {
            ...
            let _ = emitter.emit_stream_started(&loop_correlation, &gen_req.model);
            let res = gen.generate(&gen_req, &creds).await.map_err(|e| {
                let _ = emitter.emit_stream_failed(&loop_correlation, &e.to_string(), None);
                e.to_string()
            })?;
        ...
        if ai_reply.contains("[RUN_CMD:") {
            if let Some(start) = ai_reply.find("[RUN_CMD:") {
                if let Some(end) = ai_reply[start..].find("]") {
                    ...
                    let (prog, args) =
                        match crate::security::CommandPolicy::parse_and_validate(raw_cmd) {
                            Ok(p) => p,
                            Err(e) => {
                                let _ = app.emit(
                                    "chat-chunk",
                                    format!("> 🛡️ **COMMAND BLOCKED**: {}\n\n", e),
                                );
                                messages.push(ChatMessage {
                                    role: "user".to_string(),
                                    content: format!("Command Blocked: {}", e),
                                });
                                continue;
                            }
                        };

                    let (risk, requires_approval) =
                        match crate::security::CommandPolicy::evaluate_risk(&prog, &args) {
                            Ok(eval) => eval,
                            Err(e) => {
                                let _ = app.emit(
                                    "chat-chunk",
                                    format!("> 🛡️ **POLICY REJECTION**: {}\n\n", e),
                                );
                                messages.push(ChatMessage {
                                    role: "user".to_string(),
                                    content: format!("Policy Rejection: {}", e),
                                });
                                continue;
                            }
                        };
                    ...
                    if requires_approval {
                        ...
                        messages.push(ChatMessage {
                            role: "user".to_string(),
                            content: format!(
                                "Tool proposal created (ID: {}). Awaiting human operator approval.",
                                proposal.proposal_id
                            ),
                        });
                        continue;
                        ...
                        match crate::security::CommandPolicy::execute(
                            &prog,
                            &args,
                            Some(&work_path),
                        ) {
                            Ok(res) => {
                                let _ = app.emit(
                                    ...
                                    format!(
                                        ...
                                        res.output.chars().take(400).collect::<String>()
                                    ),
                                );
                                messages.push(ChatMessage {
                                    role: "user".to_string(),
                            ...
                            Err(e) => {
                                let _ = app
                                    .emit("chat-chunk", format!("> ❌ Execution Error: {}\n\n", e));
                                messages.push(ChatMessage {
                                    role: "user".to_string(),
        ...
        if ai_reply.contains("[READ_FILE:") {
            if let Some(start) = ai_reply.find("[READ_FILE:") {
                if let Some(end) = ai_reply[start..].find("]") {
                    ...
                    let res_str = match crate::security::PathSandbox::verify_containment(
                ...
                if let Some(end) = ai_reply[start..].find("]") {
                    ...
                    match parsed_res {
                        Ok(parsed) => {
                            ...
                            match crate::browser_tools::execute_browser_tool(
                                app.clone(),
                                tool_name,
                                args,
                                browser_state.clone(),
                            ...
                            .await
                            {
                                Ok(res) => {
                                    ...
                                    let _ = app.emit(
                                        ...
                                        format!(
                                            "> 📋 Result ({}ms):\n```json\n{}\n```\n\n",
                                            res.duration_ms,
                                            res_str.chars().take(800).collect::<String>()
                                        ),
                                    );
                                    messages.push(ChatMessage {
                                        role: "user".to_string(),
                                ...
                                Err(e) => {
                                    let _ = app
                                        .emit("chat-chunk", format!("> ❌ Tool Error: {}\n\n", e));
                                    messages.push(ChatMessage {
                                        role: "user".to_string(),
                                        content: format!("Browser Tool Error: {}", e),
                                    });
                                    continue;
                                }
                            }
                        ...
                        Err(e) => {
                            let _ = app.emit(
                                "chat-chunk",
                                format!("> ❌ Malformed Tool Request: {}\n\n", e),
                            );
                            messages.push(ChatMessage {
                                role: "user".to_string(),
                                content: format!("Malformed Tool Request: {}", e),
                            });
                            continue;
                        }
                    }
